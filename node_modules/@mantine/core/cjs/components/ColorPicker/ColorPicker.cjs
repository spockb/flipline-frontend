...
const ColorPicker = factory.factory((_props, ref) => {
  ...
  const formatRef = React.useRef(format || "hex");
  ...
  hooks.useDidUpdate(() => {
    if (typeof value === "string" && parsers.isColorValid(value) && !isScrubbingRef.current) {
      setParsed(parsers.parseColor(value));
    }
  }, [value]);
  hooks.useDidUpdate(() => {
    formatRef.current = format || "hex";
    setValue(converters.convertHsvaTo(formatRef.current, parsed));
  }, [format]);
